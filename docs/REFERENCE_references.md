...
- **Thrun, S., Burgard, W., & Fox, D. (2005).** *Probabilistic Robotics*. MIT Press. — the Extended Kalman Filter and its use for vehicle state estimation (Ch. 3). Used in `src/av_integrity/estimation/ekf.py`.
...
- **Bar-Shalom, Y., Li, X.-R., & Kirubarajan, T. (2001).** *Estimation with Applications to Tracking and Navigation*. Wiley. — the innovation, the Normalized Innovation Squared (NIS), and the chi-square consistency test used to flag inconsistent measurements. Used in `src/av_integrity/integrity/detector.py` (`InnovationMonitor`).
