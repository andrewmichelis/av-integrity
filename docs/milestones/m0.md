...
- An **Extended Kalman Filter** that predicts motion from the IMU and corrects it
  with GPS and wheel speed. See `av_integrity/estimation/ekf.py`, which is
  documented for a first-time reader.
- Clean, written **interface contracts** between the parts (`interfaces/`), and a
  small harness that measures accuracy.
...
**0.09 m/s**. Run `python run_demo.py` to reproduce it and see the plot.
