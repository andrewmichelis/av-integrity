...
outward, not alphabetically, so it reads like an explanation. Terms marked
*(from M1)* are defined here for context but are built in a later milestone; M0
implements everything up to and including the innovation.
...
surprise that either means our estimate was off or the sensor is wrong. M0 already
computes and records the innovation for every correction; later milestones judge
it.
...
untrustworthy. *(from M1: the integrity check that uses it.)*
...
almost never trips it, keeping false alarms rare. *(from M1.)*
...
inconsistent, and knowing it is the GPS rather than the wheels. *(built from M1
onward.)*
...
and can measure detection honestly. *(M0 runs the nominal scenario; fault
injection begins at M1.)*
